...
$COMPOSE exec -T postgres pg_dumpall -U vendaflow | gzip > "$BACKUP_DIR/pg-$STAMP.sql.gz"

echo "[backup] volumes (minio + evolution)…"
docker run --rm \
  -v vendaflow_minio_data:/data/minio:ro \
  -v vendaflow_evolution_instances:/data/evolution:ro \
  -v "$BACKUP_DIR":/backup \
